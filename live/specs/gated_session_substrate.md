...
> Two findings, reached after the close ritual that day, made the Oracle a solution without a consumer:
> 1. **Latency: an Oracle-as-Tetraform-session cannot answer in-window.** Read in app.py: SESSION_END is hard-refused before cycle 2 (the two-cycle adversarial floor), and each cycle is at minimum two sequential model calls (model_a Researcher -> model_b Challenger), so a grounded answer costs 4+ sequential frontier calls plus corpus reads — far beyond the <=60s you_there window. If ever built, the Oracle MUST be async (asker sends question+corr_id, keeps working, retrieves by corr_id later; the step-1 columns are that envelope). Synchronous is structurally impossible.
> 2. **No consumer.** Workers never needed it this session — they ground by reading the corpus directly and catch their own mistakes (the whole step-1 chain proved this). A pure-routing coordinator does not need it — routing is deterministic boolean checks against the ledger, not contestable grounding (bolting the Oracle onto the coordinator was an over-build, stripped twice). And the design chair cannot use an async multi-cycle service to ground a live sentence to the operator — that is the assertion rule's job. So the async Oracle session has no caller.
> THEREFORE: the Oracle process build (steps 2+) is SHELVED. The step-1 mailbox plumbing (question/answer kinds, corr_id/citations/confidence columns) is harmless and STAYS. The real leash is the ASSERTION RULE (below) + operator backstop for the design chair, and the already-working courier/ledger/peer-review discipline for the workers. **Do not resume the Oracle build without first naming a concrete consumer that needs adversarial grounding and can consume an async answer.** The COORDINATOR (shepherd-lineage code loop) is unaffected and remains the live build target toward the single-pane goal.
