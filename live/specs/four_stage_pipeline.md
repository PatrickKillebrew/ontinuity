...
The composed proposal is the pipeline's product. Register: plain-language, professional, minimal verbosity, structured as the stages (problem-as-defined → decomposed → composed → how it fits → path to delivery → what we need from the client). Investment/price is handled by the operator directly, NOT baked into the generated proposal. Branding: gold octagon-in-octagon mark inline with the wordmark, warm gold #c9a84c (index.html brand).
