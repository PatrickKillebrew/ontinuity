...
*Status: BUILD SPEC (not deployed). [Control currency note, post-authoring: courier allowlist is 19 ops as of 2026-06-29 (mailbox_purge, backup_db, seed_tenant, deploy, bootstrap_gate added since); CHECK 1 canonical count updated to 12 below. This is exactly the gate-constant-currency obligation §6 close-ritual (a) describes.] Authored by worker1 (claude:opus-4.8) under block BOOTGATE-1, dispatched by control. Grounds: live/PUNCH_LIST.md HIGH item "Verified bootstrap gate"; OPERATING_MANUAL.md (open ritual, close ritual, scoped operations / operations_ledger audit spine, cold-boot onboarding); corpus operations_ledger schema. Do not deploy from this file; it specifies the build for operator sign-off.*
...
### CHECK 2 — QUEUE
- Probe: read live/agent_queue.md; the now-state is the LATEST touch-point fold — an H2 headed `## CURRENT-STATE TOUCH POINT — …` (or `## FOLD — …`) containing exactly one `**NEXT…**` marker. The curated ACTIVE head at the top of the file is history (frozen at Dweller lap 3) and is NOT the current next action.
- Returned fact: the single current NEXT-SEAT action, stated in one line.
- Deterministic pass condition: the latest touch-point fold parses to exactly one NEXT marker with one bounded action (inline on the marker line, or a single bullet beneath it) AND the seat emits exactly one next-action line (the check ratifies non-empty + single-line, not the semantics — a human/control still owns whether it is the RIGHT action). An empty or unparseable queue head FAILS.
- Failure message: `NOT ORIENTED [CHECK 2 QUEUE]: latest touch-point fold missing/unparseable or NEXT not exactly one — no current next action to orient onto.`
...
- Returned fact: per-invariant token-coverage + manual-ratification findings.
...
- KNOWN GAP (finding -> control): invariant (a) "no self-poll" is not yet stated verbatim in OPERATING_MANUAL.md (this is why control drifted on it). Until the manual carries it, (a) has no manual probe and is matched against the canonical statement only; the manual MUST add it (manual-currency). Build-time the manual gains the line and (a) gets a real probe.
...
- OPEN RITUAL: today the open ritual is the instructed orientation (search queue folds, read conversation records, follow refs). The gate is the VERIFIED, machine-checked floor UNDER it: a seat runs the gate first (proves manual/queue/corpus/hands/engine + mechanics), THEN runs the human-judgment open ritual (the semantic re-grounding the gate deliberately does not attempt — the gate ratifies that the queue head is non-empty and single-line, it does not decide it is the right action). Pass the gate to earn the right to reason; run the open ritual to reason well. For a cold seat the gate replaces the ad-hoc "find your hands" checks in COLD-BOOT ONBOARDING with a deterministic pass/fail.
...
1. Sandbox-local gate runnable (the SIX checks incl. CHECK 6 MECHANICS, seat-role parameterized {control, worker}, structured result) — usable by control + workers immediately, no deploy. [BUILT: BOOTGATE-2, box live/bootstrap/gate.py — self-test oriented:true all six pass; deliberate CHECK 3 / key / CHECK 6 failures demonstrated.]
