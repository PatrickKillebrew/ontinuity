...
Allowlist (live, 24 ops) — read the live list from `__probe__`; `describe` returns every op's schema, `orient` is the per-task open ritual as a ledger row, `close_gate` is the close ritual as a ledger row. The count is bootstrap-gate CHECK 1 (manual == live) and moves in the same close as any OP_ALLOWED change. Engine deploys: MAIN is NOT deployed by pushes to main. Railway's watch pattern is `/deploy/**` (set 2026-09-16), so MAIN deploys only when `deploy/main.txt` is committed with the target sha and a message — every MAIN deploy is a deliberate, messaged commit (the `deploy` op / the operator writes it). A push that touches only app.py is a commit, not a deploy.
