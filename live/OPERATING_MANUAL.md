...
3. agent_queue.md fold — write the session's narrative fold (what was built, what was learned, what reversed), keyed on the same shas/receipts. FORMAT (CHECK 2 of the bootstrap gate parses it): head the fold `## CURRENT-STATE TOUCH POINT — <date> (<seat>)` and include exactly one line beginning `**NEXT` — the single next action inline on that line, or as one bullet beneath a bare `**NEXT**`. A fold in any other shape is invisible to the gate, and the next seat orients onto a stale action while reporting oriented.
...
THE CONTRACT (L6.5): at distillation the seat registers the punch-list slice it takes on (`contract` op: set/resolve/get; VERIFIABLE items close on a commit sha in the session's window or a ledger op_id, JUDGED items on the operator's ruling verbatim, CARRIED with a note). close_gate CHECK 1 reconciles it; exploration-only sessions (no writes, no contract) close by explicit declaration the gate verifies. `/agent/handoff` returns the receipt.
...
Allowlist (live, 25 ops) — the courier allowlist is whatever `__probe__` returns (never memorize a count); `describe` returns every op's schema, `orient` is the per-task open ritual as a ledger row, `close_gate` is the close ritual as a ledger row (nine checks; closed:true closes the seat session and revokes its key), `contract` is the seat's contract (set/resolve/get). This number is checked by the bootstrap gate (CHECK 1: manual == live) and must move in the same close as any OP_ALLOWED change. Engine deploys: MAIN is NOT deployed by pushes to main. Railway's watch pattern is `/deploy/**` (set 2026-09-16), so MAIN deploys only when `deploy/main.txt` is committed with the target sha and a message — every MAIN deploy is a deliberate, messaged commit (the `deploy` op / the operator writes it). A push that touches only app.py is a commit, not a deploy.
...
## STALE WORKING COPIES — pull live before editing, verify SHA before commit
Distinct from the /mnt/project snapshot rule: this is about a copy YOU pulled earlier in the same session going stale because live moved under you (a deploy, another session, the operator). RULE: re-pull the file live via the authed api.github.com raw endpoint as the FIRST step of any edit — never edit a copy carried across turns. Before committing, re-fetch the current blob SHA and confirm your base still matches; if it moved, rebuild on the new base. Two stale-base near-misses were caught this way; the cost of skipping is silently reverting someone else's commit.
