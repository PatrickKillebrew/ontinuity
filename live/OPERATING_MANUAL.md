...
Allowlist (live, 24 ops) — read the live list from `__probe__`; `describe` returns every op's schema, `orient` is the per-task open ritual as a ledger row, `close_gate` is the close ritual as a ledger row. The count is bootstrap-gate CHECK 1 (manual == live) and moves in the same close as any OP_ALLOWED change. Engine deploys: main is NO LONGER auto-deployed by Railway (watch pattern set 2026-09-16); MAIN deploys only through the `deploy` op or an explicit serviceInstanceDeploy — a push to main is a commit, not a deploy.
