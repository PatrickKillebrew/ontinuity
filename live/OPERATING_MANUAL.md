...
- ChatGPT Work admission check: code/shell networking is separate from browser access. Before diagnosing Railway, confirm the caller platform admitted the reference HTTPS request. A pre-HTTP denial is WORK_EGRESS_DENIED, not an Ontinuity or credential failure. Model seats use only their short-lived bearer capability; diagnostic and service roots remain with the operator and server processes.
