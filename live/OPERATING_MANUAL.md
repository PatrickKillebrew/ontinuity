...
- POST /op/deploy {action, target, signoff_block_id, commit_sha} — exact-object, two-party gated. All four fields are required strings; action is `start|status`, target is `main|farm`, and commit_sha is exact lowercase 40-hex. Query parameters, alternate content types, duplicate keys, missing/extra fields, and caller-selected provider facts are refused at MAIN and the box. Proposal and signoff must carry the identical canonical ref `deploy:v1:<main|farm|both>:<40-hex-commit>`. The deploy-only capability is independently bound to the same block, commit, and target scope; MAIN checks it before relay, and the box derives and checks the signed ref before provider action. Under identity-bound auth the caller must be the distinct-seat signer. Provider credentials remain server-side. The clean non-author signer deploys exact reviewed bytes; an author/corrector may not.
...
Provider configuration is operator-maintained and remains outside model hands.
The trusted box-side `deploy` operation accepts only a signed-off exact commit and
logical `main|farm` target; its internal provider adapter owns every credential,
endpoint, request shape, service binding, timeout, redirect, status, and bounded
failure-log read. A model never reconstructs or selects those details. A variable
change remains a separate operator-reviewed configuration act. DESIGN RULE: keep
adversarial roles on different training lineages so their error geometries differ.
...
The credential-bootstrap vault is provider-managed server configuration. Its root
recovers repository, diagnostic, and mailbox credentials for an operator, but is
never a seat credential. This is distinct from the engine reading runtime model
keys from its own environment; do not mistake either mechanism for model hands.

Vault recovery is an operator-only action outside model boot and normal model
operation. The model receives neither the provider token nor recovered secrets.
The trusted deployment adapter reads its own server environment and never accepts
credentials or provider details in a caller body. Provider-specific recovery
instructions belong only in the trusted server runbook, not in a model packet.
