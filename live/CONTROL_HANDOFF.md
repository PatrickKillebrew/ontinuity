...
# Updated 2026-09-21 by Control (claude.ai-chat:claude-fable-5.1) — pitch repo cleanup session: papers/ move, prompts to museum, AGPL-3.0 LICENSE + NOTICE, architect README, business-owner landing page (commits 90ec391 e00a3c6 5efffbd 766d8c9 fa3a94f); NO engine, box, op or manual change; single next action unchanged (Phase 3). Prior: 2026-09-16 (day close) by Control (claude.ai-chat:claude-fable-5.1) — LOCKDOWN LIST COMPLETE on both installs (L1–L9 incl. L6.5); MAIN deploys via deploy/main.txt marker; both engines at 2be8b89. Earlier same day: (night) by Control (claude.ai-chat:claude-fable-5.1) — L6/L4b/L7 live; STAFFING check; first Researcher-seat sessions (no work product; two defects found); engine fix MERGED to main a0c61f0 under operator directive; both engines idle. Prior: 2026-09-15 evening. Prior 2026-09-14: reflex-layer research shelved to V2-E. Prior update 2026-09-13 (post-GPT rollback reconciliation; B1 to museum/).
...
- Recovery + mapping record: `live/conversations/2026-09-12_recovery-mapping-two-product-packaging.md`; maps: `live/ONTINUITY_MASTER_SYSTEM_MAP.md`, `live## SINGLE NEXT ACTION
PHASE 3 — SHIP THE SEAT: a non-Claude model boots install two from live/CONTROL_QUICKBOOT.md v3.1 (ontinuity-two) with only LLaves.txt attached; it must boot through bootstrap_gate (seven checks, session, key), register a contract for a small task with `contract`, do the task, and close through close_gate (closed:true). Read the receipt first: GET {engine}/agent/handoff (X-Diag-Key) -> `seat` block. Then Phase 4 (Cornel's install on his accounts).
STATE (2026-09-16 day close): both installs at 2be8b89, 25 ops, gates L1–L9 live, roles alive; MAIN deploys ONLY by committing deploy/main.txt (Railway watches /deploy/**); engine-two by explicit serviceInstanceDeploy. Work order live/specs/ritual_lockdown_punchlist.md: all DONE. Records: 2026-09-16_day-close, 2026-09-16_night-close, 2026-09-15b, 2026-09-15.

IX-CONTRACT-1 recorded. Operator install: box runs the old box_ops, manual says 21 — reconcile via L9 (also gate Railway's auto-deploy). Records: 2026-09-16_night-close (this), 2026-09-15b, 2026-09-15.
