...
## LAP 2 AMENDMENTS — June 6, 2026 (sessions 16-01-07 + 20-52-28, receipts #9-#10)
**CLOSED: PyGithub/push-path item** — full repair arc proven: module installed into the service venv (system install was insufficient — the service runs its own interpreter), invocation whitelisted with required filepath argument, credentials added to config.json, lookup key typo corrected, push executed and the artifact independently verified in the public repo (live/sessions/push_test.txt, HTTP 200). Sessions can now push artifacts to the record — "artifact pushed" is a satisfiable, verifiable contract criterion class.
**CLOSED: exposed-credential incident** — classic ghp_ token found hardcoded in the public live/misc/push_to_github.py; token deleted by operator; replaced with fine-grained single-repo token in config.json. Deferred by operator choice: rotating the new token after it appeared in a terminal paste; remove live/misc/push_to_github.py from the repo.
**NEW: script exit-code hygiene** — push_to_github.py prints [SYNC] ERROR but exits 0; PASSED on this command is unreliable without reading stdout. Fix: nonzero exits on failure.
**NEW: token custody** — script should read its token from an environment variable or config only; no credential-bearing .py may live in a path the script itself can push.

## LAP 3 AMENDMENTS — June 6, 2026 (session 2026-06-06_21-55-43, receipt #11, CERTIFIED)
**Item 6 friction audit: CLOSED CERTIFIED.** The lap-1 audit re-ran with every figure established through the new DB_QUERY evidence channel (deploy 14) — five injected query results, zero unverifiable claims. One challenge upheld against the Researcher (aggregate correlation overclaimed as calibration — accepted and requalified with an explicit overlap query); one challenge rejected in the Researcher's favor. New audit finding: signal-0 and signal-1 distributions interleave per-row (38% of signal-0 rows carry nonzero challenges, including 15 rows at 7+ challenges scored 0) — fix package item 6b gains a fifth element: define the 0/1 boundary against explicit row-level features.
**Infrastructure shipped this arc (deploys 14-16, agent-committed):** DB_QUERY corpus evidence channel; STOP wakes the mailbox; missing json import; engine console ring buffer at /diag/console; PRE_SESSION questions route to the session starter via the mailbox (proven live — the lap-3 contract was authored through agent-answered questions).
**NEW direction (operator decision, June 6 evening):** VPS hands move to a coding agent (Claude Code or any capable agent — the seat protocol is agent-agnostic by design); tokens, credentials, and judgment modals remain human. 
**NEW session type: the Dweller lap** — periodic sessions whose contracted objective is the queue itself: read, weigh recent receipts, propose the rebuilt/reprioritized queue, defend the reordering under challenge, commit via queue_update with the session receipt as provenance. Queue curation moves under the glass. First Dweller lap should rebuild this queue, which has grown by append all day.
