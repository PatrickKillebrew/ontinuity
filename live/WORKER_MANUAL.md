...
- The current boot packet supplies the shared DIAG_KEY as an interim credential. Never place it in a URL, transcript result, staged file, or committed file. Seat names in request bodies remain self-asserted; that ceiling stays explicit (per-identity keys are a future item).
