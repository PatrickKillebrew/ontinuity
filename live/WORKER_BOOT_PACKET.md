...
2. read_repo `live/CONTROL_HANDOFF.md` and the latest CURRENT-STATE TOUCH POINT fold at the tail of `live/agent_queue.md`; state the single current next action and the block-specific dependency in one line. Do not read the chronological queue's oldest head as current state.
