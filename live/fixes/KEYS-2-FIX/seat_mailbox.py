...
def mailbox_send():
    ...
    body      = b.get("body")
    # KEYS-2-FIX (closes SECAUDIT-1 Q1 / signoff_keys.md REJECT): the SENDER identity
    # (from_seat/from_lineage) is DERIVED from the authenticating key, never trusted
    # from the body — the same _trusted_seat chokepoint the other 4 routes use. When a
    # per-identity key authenticates, the body from_seat is only a REQUEST: a mismatch
    # is rejected (409), it can never set the sender to another seat. In shared-key
    # back-compat mode _trusted_seat returns the body seat (honest-but-asserted), so
    # existing callers keep working until keys are issued. NOTE: author_seat/
    # author_lineage are NOT migrated — they are a claim about who authored a THIRD-PARTY
    # reviewed artifact (e.g. control dispatching a worker's proposal), not an assertion
    # of the sender's own identity, so they remain caller-supplied by design.
    auth_seat, auth_lin, authed = _trusted_seat(body_from_seat, b.get("from_lineage"))
    if authed:
        if body_from_seat and body_from_seat != auth_seat:
            return jsonify({"error": "from_seat does not match authenticated identity",
                            "authenticated_seat": auth_seat, "body_from_seat": body_from_seat}), 409
        from_seat = auth_seat
        from_lineage = auth_lin
    else:
        from_seat = body_from_seat
        from_lineage = (b.get("from_lineage") or "").strip() or None
    ...
    try:
        ...
        c.execute("""INSERT INTO seat_mailbox
            ...
            (msg_id, from_seat, from_lineage, to_seat, kind,
             ...
             (b.get("author_lineage") or (from_lineage or "")).strip() or None))
        c.commit(); c.close()
