...
def call_projenius(function_tag, **kwargs):
    ...
    has_projenius = bool(projenius_cfg.get("api_key") or projenius_cfg.get("url"))
...
def run_session_loop(objective, start_fresh=False):
            ...
            if b_tag == "CHALLENGE":
                ...
                if ruling:
                    ...
                    if "UPHOLD" in ruling.upper():
                        ...
                        socketio.emit('routing_action', {
                            'type': 'parietal',
                            'message': f'Claim ruled against — added to Researcher session memory.'
                        })
    ...
    if not distilled:
        ...
        delta_log = ""
        if parietal_distilled:
            # Extract Delta Log from distillation output
            for line in parietal_distilled.split("\n"):
                if line.strip().startswith("Delta Log:"):
                    delta_log = line[10:].strip()
                    break
            if not delta_log:
        ...
        if delta_log:
            ...
            synthesize_result = run_projenius_synthesize(delta_log, knowtext)
            if synthesize_result:
                socketio.emit('routing_action', {'type': 'parietal', 'message': 'Established Results Ledger updated.'})
            else:
                socketio.emit('routing_action', {'type': 'parietal', 'message': 'Projenius SYNTHESIZE skipped — model not configured or no response.'})
...
def pre_session_then_start(obj, start_fresh=False):
    ...
    knowtext = load_file(CONFIG["knowtext_path"]) or ""
    orient_result = run_projenius_orient(obj, knowtext)
    orient_context = ""
    if orient_result:
        orient_context = orient_result
